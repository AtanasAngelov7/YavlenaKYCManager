...
Run `dist\installer\YavlenaKYCManager-Setup-0.1.1.exe`, then open **Yavlena KYC Manager** from the Start menu or its optional desktop shortcut. The installer is currently unsigned because this is a local POC, so Windows may display an unknown-publisher warning.
